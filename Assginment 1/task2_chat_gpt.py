...
#     def __init__(self, video_path, output_bg='background.mp4', output_fg='foreground.mp4', history=500, var_threshold=16, alpha=0.01, num_gaussians=5):
...
#         fg_mask = np.ones((height, width), dtype=np.uint8) * 255  # Initialize as foreground (white)
...
#             # Mark background pixels as black
#             fg_mask[match] = 0
...
#             bg_image = np.mean(self.background_model[..., 0], axis=2).astype(np.uint8)
...
#         print("Processing complete. Background and foreground videos saved.")

# # Example usage:
# bgs = StaufferGrimsonBGS('video.mpeg')
# bgs.process_video()
